...
- To get the latest code without recloning, run:
  ```bash
  git pull origin main
  ```
  (or `git pull origin <branch-name>` if testing a PR branch).
- If STL still does not appear, open browser DevTools (`Cmd+Option+I`) and check Console errors for blocked CDN imports or parse failures.
- Use **Reframe camera** after loading if your STL is very small/large and appears out of frame.
## Current status
